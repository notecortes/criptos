...
const DEFAULT_CONFIG = {
    selectedCryptos: ['bitcoin', 'ethereum', 'binancecoin', 'cardano', 'solana', 'polkadot'],
...
class CryptoTracker {
    ...
    createCryptoCard(crypto) {
        ...
        card.innerHTML = `
            <div class="crypto-header">
                <img src="https://assets.coingecko.com/coins/images/${crypto.imageId}/small/${crypto.id}.png" 
    ...
    async initializeChart() {
        ...
        AppState.priceChart = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: `${cryptoData?.name || 'Bitcoin'} (${cryptoData?.symbol || 'BTC'})`,
                    data: prices,
                    borderColor: '#3498db',
    ...
    async saveConfiguration() {
        ...
        CONFIG.updateInterval = parseInt(document.getElementById('priceInterval').value);
        CONFIG.chartUpdateInterval = parseInt(document.getElementById('chartInterval').value);
    ...
    async updateChart() {
        ...
        AppState.priceChart.data.datasets[0].label = `${cryptoData?.name || 'Bitcoin'} (${cryptoData?.symbol || 'BTC'})`;
